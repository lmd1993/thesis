...
% Since the working pipeline with other libraries except machine learning should be similar to the previous introduced pipeline, we only describe one machine learning example here. 
...
\vspace{0.5em}
\ex{3} - Training of Linear Regression model with LLib.
\bldl
//\ Import\ data. 
\\
...
% val\ model = lr.genDF(dfTrain, session) \\
...
\vspace{0.5em}
\ex{4} - Collaboration between LLib and other Spark libraries.
\bldl
val\ MLM = new\ MLM()\\
MLM.setDirection(MCol = "Member", ProfitCol = "Bonus")\\
MLM.setSecDirection(MCol = "Member1", M2Col = "Member2") \\
...
\vspace{0.5em}
\ex{5.1} - Development with LFrame:  Conversion from DataFrame to LFrame.
\bldl
...
\vspace{0.5em}
\ex{5.2} - Development with LFrame:  Unary operations with SG application.
\setcounter{myrow}{0}
\\
...
\vspace{0.5em}
\ex{5.3} - Development with LFrame:  $N$-ary operations with MLM Bonus application.
\setcounter{myrow}{0}
\\
